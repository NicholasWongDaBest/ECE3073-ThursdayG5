...
extern volatile int start_vga;
extern volatile int start_spi;
extern volatile int * KEY_GPIO;
...
asm ( ".set noat" ); 					// Magic, for the C compiler
asm ( ".set nobreak" ); 				// Magic, for the C compiler
asm ( "subi sp, sp, 128" ); 			// make space on the stack(stack pointer)
asm ( "stw et, 96(sp)" ); 				// save exception temporary (r24)
...
asm ( "subi ea, ea, 4" ); 				// if external must decrement
										...
										// will be run
asm ( "SKIP_EA_DEC:" );
asm ( "stw r1, 4(sp)" ); 				// Save all registers (to preserve all the data as interrup will happen anytime)
asm ( "stw r2, 8(sp)" );
asm ( "stw r3, 12(sp)" );
asm ( "stw r4, 16(sp)" );
asm ( "stw r5, 20(sp)" );
...
void interrupt_handler(void)
{
	...
	if(ipending & 0x8){
		// Read the edge capture register to see which key was pressed
		int edge_capture = *(KEY_GPIO + 3);

		if(edge_capture & 0x1) start_spi = 1;
		if(edge_capture & 0x2) start_vga = 1;

		*(KEY_GPIO + 3) = 0x0; // Clear interrupt flag
	 }
