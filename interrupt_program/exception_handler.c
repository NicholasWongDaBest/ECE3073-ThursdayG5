...
void interrupt_handler(void)
{
    ...
    // Read the Control Register 4 (ipending) to see who interrupted
    NIOS2_READ_IPENDING(ipending);

    // PIO_7 (Buttons) is IRQ 3. Check bit 3 (0x8)
    if (ipending & (1 << PIO_7_IRQ))
    {
        // Read the edge capture register for buttons 0 and 1
        int edge_capture = IORD_ALTERA_AVALON_PIO_EDGE_CAP(PIO_7_BASE);
        ...
        IOWR_ALTERA_AVALON_PIO_EDGE_CAP(PIO_7_BASE, 0x3);
    }

    // You can add more 'if' blocks for other IRQs here (like IRQ 2 for Accelerometer)
