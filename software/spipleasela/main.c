...
volatile int * PIXEL_EXPORT = (volatile int *)0x040000a0;
volatile int * ADDR_EXPORT  = (volatile int *)0x04000080;
volatile int * WREN_EXPORT  = (volatile int *)0x04000070;
...
int main()
{
    ...
    while(1)
    {
    	...
    	if(start_spi){
			...
			if(pattern_hits == 1){
				IOWR_8DIRECT(DRAM_WRADDR,offset,rx);
				offset ++; //don't need to +4 as we only writing 1 byte
			}
